...
def get_google_sheets_client():
    ...
    try:
        ...
        st.info("⚠️ Streamlit Cloudの設定でGoogle認証情報を追加してください")
        return None

def get_spreadsheet_id():
    """スプレッドシートIDを取得（secretsまたはデフォルト）"""
    try:
...
def create_spreadsheet_if_needed(sheets_client):
    ...
    if not spreadsheet_id:
        ...
        try:
            ...
            st.info("👆 このIDを .streamlit/secrets.toml に追加してください")
            return new_id
        except Exception as e:
...
if sheets_client:
    ...
    if spreadsheet_id:
        ...
        with tab3:
            ...
            with st.expander("➕ 新規ポジション登録", expanded=False):
                ...
                # 損切り理由
                stop_reasons = get_reason_list(sheets_client, spreadsheet_id, 'stop_loss')
                if len(stop_reasons) > 0:
                    stop_reason = st.selectbox("損切り理由", stop_reasons['detail'].tolist(), key="stop_reason")
                else:
                    stop_reason = st.text_input("損切り理由", key="stop_reason")
                
                entry_notes = st.text_area("メモ", key="entry_notes")
                
                if st.button("✅ 登録", use_container_width=True):
                    if entry_ticker and entry_name and entry_price > 0 and entry_qty > 0:
                        new_row = {
                        ...
                        if st.session_state.get(f"closing_{idx}", False):
                            with st.form(f"close_form_{idx}"):
                                ...
                                if submit and exit_price > 0:
                                    ...
                                    if len(df_closed) == 0:
                                        df_closed = pd.DataFrame([closed_row])
                                        write_sheet(sheets_client, spreadsheet_id, 'closed_trades', df_closed)
                                    else:
                                        append_to_sheet(sheets_client, spreadsheet_id, 'closed_trades', closed_row)
                                    
                                    # active_tradesから削除（is_activeを0に）
        ...
        with tab5:
            ...
            st.divider()
            st.subheader("スプレッドシート情報")
            st.code(f"Spreadsheet ID: {spreadsheet_id}")
            st.caption("このIDを .streamlit/secrets.toml に保存してください")
    ...
    3. JSONキーをダウンロード
    4. `.streamlit/secrets.toml` に認証情報を追加
    
    詳細は README_SHEETS.md を参照してください。
    """)
